"""
Task 1: Hyperparameter Tuning Experiments
Trains 10 DQN configurations on ALE/Pong-v5 using Stable Baselines3.
Each configuration explores a distinct region of the hyperparameter search space
to observe the effect of learning rate, gamma, batch size, and epsilon on agent behavior.

Run all 10 experiments:
    python train_alliance.py --mode experiments

Run the best config (Exp3 XLBatch) as a standalone final model:
    python train_alliance.py --mode final
"""
...
# Each configuration varies one or more of the following dimensions:
#   - Learning rate: controls the step size of gradient updates
#   - Gamma: discount factor determining how much future rewards are valued
#   - Batch size: number of transitions sampled per training update
#   - Epsilon (start, end, decay): controls the exploration-exploitation tradeoff
# ---------------------------------------------------------------------------
HYPERPARAMETER_CONFIGS = [
    {
        "name": "Exp1_VeryHighLR_FastDecay",
        "learning_rate": 0.005,
        "gamma": 0.99,
        "batch_size": 32,
        "epsilon_start": 1.0,
    ...
    {
        "name": "Exp2_VeryLowGamma_LargeBatch",
    ...
    {
        "name": "Exp4_MiniBatch_VeryFastDecay",
        "learning_rate": 0.0001,
        "gamma": 0.99,
        "batch_size": 8,
        "epsilon_start": 1.0,
    ...
    {
        "name": "Exp6_ZeroWarmup_ImmediateExploit",
        "learning_rate": 0.0002,
        "gamma": 0.99,
        "batch_size": 32,
        "epsilon_start": 0.2,
    ...
    {
        "name": "Exp7_HighLR_XLBatch_LowGamma",
        "learning_rate": 0.001,
        "gamma": 0.92,
        "batch_size": 128,
        "epsilon_start": 1.0,
    ...
    {
        "name": "Exp9_HighLR_MidGamma_SlowDecay",
    ...
    {
        "name": "Exp10_SmallBatch_HighGamma_LowEpsEnd",
...
class TrainingLogger(BaseCallback):
    """Logs mean reward and episode length every 1 000 timesteps."""
...
# ---------------------------------------------------------------------------
# Behavior summariser (reads the log file after training)
# ---------------------------------------------------------------------------
...
# Run all 10 experiments
# ---------------------------------------------------------------------------
def run_hyperparameter_experiments(member_name="Alliance"):
    """Run all 10 hyperparameter experiments and append results to the shared CSV and markdown table."""
    ...
    # Merge with existing results if a prior run's CSV is already present
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="DQN Hyperparameter Tuning Experiments — Alliance")
    parser.add_argument(
        "--mode",
        choices=["experiments", "final"],
        default="experiments",
        help="experiments = run all 10 configs | final = train best config only",
    )
    parser.add_argument(
        "--member",
        type=str,
        default="Alliance",
        help="Your name (used in the results table)",
    )
    ...
    if args.mode == "experiments":
        df = run_hyperparameter_experiments(member_name=args.member)
        print("\n" + df.to_string())
    else:
        # Best config: Exp3 — XL batch is the most unexplored and often
        # improves stability; use it as the "best" single run.
        print("Training best config: Exp3_XLBatch_StdLR")
        cfg = next(c for c in HYPERPARAMETER_CONFIGS if "XLBatch" in c["name"])
        model, env, _ = train_dqn_agent(
            env_name="ALE/Pong-v5",
            policy_type="CnnPolicy",
            ...
            model_save_path="dqn_model_alliance.zip",
            log_file="final_training.txt",
            experiment_name="Final_Model_Alliance",
        )
        env.close()
        print("\nDone. Model saved to dqn_model_alliance.zip")
